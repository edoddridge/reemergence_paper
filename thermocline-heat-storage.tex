...
Cambridge, 02139,
USA}
\extraaffil{Institute for Marine and Antarctic Studies, University of Tasmania,
...
\abstract{The observational record shows a substantial increase in summertime westerly winds over the Southern Ocean. This change is encapsulated by the positive trend in the summertime Southern Annular Mode (SAM) index. Enhanced summertime westerly winds have been linked to cold summertime sea surface temperature (SST) anomalies. Previous papers have suggested an advective mechanism drives this seasonal cooling. Here a new mechanism is presented in which the cooling is driven by enhanced vertical mixing that moves heat downwards, cooling the sea surface and warming the subsurface waters. The cold SSTs enhance atmospheric heat fluxes into the ocean, causing the depth-integrated ocean heat content anomaly to become positive. The subsurface heat is returned to the surface mixed layer during the autumn and winter as the mixed layer deepens, leading to anomalously warm SSTs potentially reducing sea ice cover. Observational analyses and numerical experiments support this mechanism, showing that enhanced vertical mixing drives subsurface warming and cools the surface mixed layer. Anomalous advection also contributes to the surface cooling, but the relative importance of the advective contribution is model dependent. The modeling results suggest that sea ice volume is more sensitive to summertime winds than sea ice extent, implying that enhanced summertime westerly winds may lead to thinner sea ice in the following winter. Thus, strong summertime winds could precondition the sea ice cover for a rapid retreat in the following melt season.
}
...
\section{Analysis of Observations} % (fold)
...
\section{Analysis of an idealized channel model} % (fold)
...
\section{Analysis of a global coupled model} % (fold)
